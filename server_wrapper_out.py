...
def host_model(model: Any, name: str, port: int = 5000) -> None:
    ...
    def process_request_old() -> Dict[str, Any]:
        # 保持向后兼容
        if request.method == 'GET':
            return jsonify(model.process_payload(None))
        else:
            return jsonify(model.process_payload(request.json))
    
    # 添加 OpenAI 兼容的路由
    @app.route('/v1/chat/completions', methods=['POST'])
    def process_chat_completions() -> Dict[str, Any]:
        ...
        if messages:
            ...
            if isinstance(txt, list):
                # 处理多模态输入（图片+文字）
                txt = " ".join(item["text"] for item in txt if item["type"] == "text")
        else:
            txt = ""
        
        response = model.process_payload({"txt": txt})
